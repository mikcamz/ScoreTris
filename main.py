...
def draw_mini_piece(surf, name, x, y, cell_size=18):
    ...
    for dr, dc in shape:
        px = x + dc * cell_size
        py = y + dr * cell_size
        pygame.draw.rect(surf, color, (px, py, cell_size - 1, cell_size - 1))
        pygame.draw.rect(surf, tuple(min(c + 40, 255) for c in color),
                         (px, py, cell_size - 1, cell_size - 1), 1)
...
def draw_sidebar(surf, game, font):
    ...
    # --- HOLD ---
    surf.blit(font.render("HOLD", True, WHITE), (sx, OY))
    pygame.draw.rect(surf, DARK_GRAY, (sx, OY + 22, 80, 60), 1)
    draw_mini_piece(surf, game.hold_name, sx + 6, OY + 28)

    # --- NEXT ---
    surf.blit(font.render("NEXT", True, WHITE), (sx, OY + 100))
    previews = game.preview(5)
    for i, name in enumerate(previews):
        py = OY + 125 + i * 50
        pygame.draw.rect(surf, DARK_GRAY, (sx, py, 80, 50), 1)
        draw_mini_piece(surf, name, sx + 6, py + 6, 14)

    # --- SCORE ---
    sy = OY + 390
    for label, value in [("SCORE", game.score), ("LINES", game.lines), ("LEVEL", game.level)]:
        surf.blit(font.render(label, True, WHITE), (sx, sy))
        surf.blit(font.render(str(value), True, WHITE), (sx, sy + 18))
        sy += 48

    # --- AI STATUS ---
    ax = sx + 95
    surf.blit(font.render("AI ANALYZER", True, SOFT_BLUE), (ax, OY))
    surf.blit(font.render(f"Mode: {game.search_mode.upper()}", True, WHITE), (ax, OY + 24))
    surf.blit(font.render(f"Depth: {game.lookahead_depth}", True, WHITE), (ax, OY + 44))
    surf.blit(font.render(f"Beam: {game.beam_width}", True, WHITE), (ax, OY + 64))
    surf.blit(font.render(f"Nodes: {game.last_search_nodes}", True, WHITE), (ax, OY + 84))
    eval_text = "-" if game.best_eval_current is None else f"{game.best_eval_current:.2f}"
    surf.blit(font.render(f"Best Eval: {eval_text}", True, WHITE), (ax, OY + 104))
    surf.blit(font.render(f"Review: {game.last_move_review}", True, SOFT_GREEN), (ax, OY + 124))

    # --- WEIGHTS ---
    surf.blit(font.render("WEIGHT TUNING", True, SOFT_BLUE), (ax, OY + 160))
    for i, (name, value) in enumerate(zip(game.weight_names, game.weights)):
        color = SOFT_GREEN if i == game.weight_selected else WHITE
        surf.blit(font.render(f"w{i+1} {name[:8]}: {value:+.2f}", True, color), (ax, OY + 185 + i * 20))

    # --- LEADERBOARD ---
    surf.blit(font.render("LEADERBOARD", True, SOFT_BLUE), (ax, OY + 280))
    top = game.leaderboard[:3]
    for i, item in enumerate(top):
        ...
        surf.blit(font.render(f"{i+1}. {score} ({lines}L) {day}", True, WHITE), (ax, OY + 304 + i * 32))
        surf.blit(font.render(f"    {pattern_code}", True, SOFT_GREEN), (ax, OY + 320 + i * 32))
    ...
    for line in help_lines:
        surf.blit(font.render(line, True, WHITE), (ax, hy))
        hy += 18
...
def draw_game_over(surf, font_big, font_small):
    ...
    text1 = font_big.render("GAME OVER", True, (240, 60, 60))
    text2 = font_small.render("Press R to restart", True, WHITE)
    surf.blit(text1, (WIN_W // 2 - text1.get_width() // 2, WIN_H // 2 - 40))
    surf.blit(text2, (WIN_W // 2 - text2.get_width() // 2, WIN_H // 2 + 20))
...
def main():
    ...
    das_dir = 0        # -1 left, 1 right, 0 none
    das_timer = 0
    das_charged = False
